...
user stops speaking
  ↓
VAD detects end-of-turn        (~50-150ms)
  ↓
ASR finalizes transcript        (~50-200ms)
  ↓
LLM produces first token        (~300-800ms)
  ↓
TTS produces first audio frame  (~100-400ms)
  ↓
audio plays from speaker        (~30ms playback buffer)
```
