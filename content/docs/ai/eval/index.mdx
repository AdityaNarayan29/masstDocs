...
- **Con:** lags real-time (judge calls take seconds), can't block bad deploys (only detects them after the fact), costs money per sampled request. A 500-example golden set × 3 rubric dims × Claude Sonnet judge is ~$5-15/run; multiply by your CI frequency. See [Cost engineering](/ai/cost) for batch-API discounts and caching patterns that bring this down 5×.
