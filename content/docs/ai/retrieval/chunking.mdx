...
**Recall@10:** 0.79.
**Best for:** documents with natural hierarchy (Markdown with headings, code files with functions, books with chapters). LangChain's `ParentDocumentRetriever` and LlamaIndex's `AutoMergingRetriever` both implement this.
...
Changing your chunk size from 512 to 800 means **every chunk in your vector DB is now wrong**. The embeddings were computed on different text. You have to re-index. Plan for periodic re-indexes; budget the embedding cost in advance.
