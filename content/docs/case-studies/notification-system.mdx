...
```mermaid
flowchart TD
    GW["API Gateway"] --> NS["Notification Service"]
...
```mermaid
flowchart TD
    A["1. Receive notification request"] --> B["2. Validate and enrich data"]
    B --> C["3. Check user preferences"]
    C --> D["4. Apply rate limiting"]
    D --> E["5. Queue for delivery"]
```
...
```mermaid
flowchart TD
    A["1. Consume from queue"] --> B["2. Build notification (templates)"]
    B --> C["3. Call external provider"]
    C --> D["4. Update delivery status"]
    D --> E["5. Handle retries"]
```
