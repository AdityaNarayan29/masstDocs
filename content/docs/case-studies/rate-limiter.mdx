---
title: Rate Limiter
description: Design a distributed rate limiter — token bucket vs leaky bucket vs sliding window, Redis backing, and per-user / per-API limits.
---
...
```mermaid
flowchart TB
    GW["API Gateway"] --> RL["Rate Limiter Service"]
    RL --> Redis["Redis Cluster (counters)"]
...
```mermaid
flowchart LR
    S1["Server 1"] --> Redis["Redis (shared counter)"]
    S2["Server 2"] --> Redis
    S3["Server 3"] --> Redis
