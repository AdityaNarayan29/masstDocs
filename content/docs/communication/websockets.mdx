...
sequenceDiagram
    ...
    C->>S: HTTP Upgrade Request (Upgrade: websocket)
    S-->>C: HTTP 101 Switching Protocols
    C<<->>S: WebSocket Messages (bidirectional)
```
...
```mermaid
flowchart LR
    SA["Server A (users)"] <-->|"pub"| R["Redis Pub/Sub"]
    R <-->|"sub"| SB["Server B (users)"]
```
