...
flowchart TB
    subgraph Pool["Shared Pool (100 threads)"]
        A["Service A calls (fast) — few threads"]
        B["Service B calls (SLOW!) — consumes all threads"]
        C["Service C calls (fast) — starved!"]
    end
    style B fill:#fee2e2
    style C fill:#fef3c7
```
...
```mermaid
flowchart LR
    PA["Pool A (30)"] --> SA[Service A]
    PB["Pool B (40)"] --> SB["Service B (slow)"]
    PC["Pool C (30)"] --> SC[Service C]
    style PB fill:#fee2e2
    style SB fill:#fee2e2
    style SC fill:#dcfce7
```

Pool B is saturated by slow Service B, but Pools A and C are unaffected.
...
flowchart TB
    subgraph App["Application"]
        PB["Payment Bulkhead (20)"]
        IB["Inventory Bulkhead (30)"]
        SB["Shipping Bulkhead (15)"]
    end
    PB --> PAPI[Payment API]
    IB --> IAPI[Inventory API]
    SB --> SAPI[Shipping API]
    style PB fill:#dbeafe
    style IB fill:#dbeafe
