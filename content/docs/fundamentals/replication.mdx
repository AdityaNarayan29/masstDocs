...
flowchart TD
    ...
    L -->|"Replication"| F3["Follower 3"]
    F1 --> R["Reads (distributed)"]
    F2 --> R
    F3 --> R
```
...
```mermaid
flowchart LR
    LA["Leader A (writes)"] <-->|"bidirectional"| LB["Leader B (writes)"]
    LA --> FA["Follower"]
    LB --> FB["Follower"]
```
...
sequenceDiagram
    ...
    U->>L: Write data
    U->>F: Read data
    F-->>U: Old data! (hasn't received write yet)
```
...
sequenceDiagram
    ...
    U->>FA: Read (has data ✓)
    FA-->>U: Latest data
    U->>FB: Read (missing data ✗)
    FB-->>U: Old data — appears to go backwards!
```
