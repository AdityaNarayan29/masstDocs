...
| **Command** | Each `Move` is a reversible action |
| **Memento** | `Move` stores captured piece for undo |
| **Factory** | Construct piece by type during promotion |
| **Singleton** | (not really; `Game` is one per match) |
