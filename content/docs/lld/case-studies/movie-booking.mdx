...
| **Pattern** | **Where** |
|------------|-----------|
| **State** | `SeatStatus` lifecycle (Available / Held / Booked) |
| **Strategy** | Pricing per seat type; cancellation refund policy |
| **Facade** | `BookingService` |
| **Observer** | Show countdown to hold expiry on user UI |
| **Singleton** | One `BookingService` per app |
