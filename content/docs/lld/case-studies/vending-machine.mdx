...
| **Pattern** | **Usage** |
|------------|-----------|
| **State** | Idle / ItemSelected / Dispensing transitions |
| **Strategy** | Change-making algorithm (greedy vs DP) |
| **Singleton** | `VendingMachine` per physical unit |
| **Command** | Each user action as a command (for replay/audit) |
| **Observer** | Display board updates from machine events |
